...
**KHÔNG DÙNG EMOJI** - Thay bằng custom SVG icons từ `components/icons/ThaiIcons.tsx`:

| Icon Component | Thay cho | Dùng trong |
|----------------|----------|------------|
| `ThaiIcon` | - | Selector Thai (An Nhơn, Nhơn Phong, Hoài Nhơn) |
| `CoNhonBrandIcon` | 🎯🎬 | Post header thương hiệu |
| `CommentIcon` | 💬 | Thống kê & empty states bình luận |
| `HeartIcon` | ❤️🤍 | Like button (prop `filled` cho trạng thái) |
| `EmptyIcon` | 📭 | Trạng thái không có nội dung |
| `WarningIcon` | ⚠️ | Thông báo lỗi |
| `TetModeIcon` | 🎋 | Chế độ Tết toggle |
| `LightbulbIcon` | 💡 | Hướng dẫn/Tips |
| `LoadingIcon` | 🔄 | Trạng thái loading (có animation) |
| `VideoIcon` | 📺 | Thống kê video |
| `BanIcon` | 🚫 | Bình luận bị cấm |
| `UserIcon` | 👤 | Thông tin người dùng |

```tsx
// Cách sử dụng
import { HeartIcon, CommentIcon } from '../components/icons/ThaiIcons';
...
**Endpoint**: `POST /api/upload/cau-thai`

| Field | Type | Description |
|-------|------|-------------|
| image | File | Ảnh câu thai (JPEG, PNG, GIF, WebP), max 5MB |

**Response**:
```json
{
  "success": true,
  "imageUrl": "/uploads/cau-thai/1707123456-uuid.png",
  "filename": "1707123456-uuid.png",
  "size": 102400
}
```
...
- Production: `/opt/conhon/uploads` (persistent)
- ENV: `UPLOAD_DIR=/app/uploads`

> [!IMPORTANT]
> **Cấu trúc thư mục trên VPS:**
> ```
> /opt/conhon/                        ← THƯ MỤC DỰ ÁN (clone từ GitHub)
> ├── backend/
> ├── frontend/
> ├── docker-compose.yml
> └── .env
> 
> /var/lib/docker/volumes/            ← DOCKER VOLUMES (persistent data)
> └── conhon-production_uploads_data/
>     └── _data/
>         └── cau-thai/               ← THƯ MỤC ẢNH UPLOAD
>             ├── 170712345-uuid.png
>             └── ...
> ```
> 
> **Truy cập ảnh từ browser:** `https://your-domain.com/uploads/cau-thai/filename.png`
...
```
Push to main/master → GitHub Actions → SSH to VPS → docker-compose up --build
```
...
| `VPS_USER` | SSH user (e.g., `root` hoặc `ubuntu`) |
| `VPS_SSH_KEY` | Private SSH key (Ed25519 recommended) |

### 10.3 VPS Initial Setup

Run once on fresh Ubuntu VPS:
```bash
...
- [ ] uploads_data volume backup strategy

---

*Phiên bản 3.1 - Thêm CI/CD & File Upload - 05/02/2026*
